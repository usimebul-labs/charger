        ...
        <div className="flex items-center justify-between gap-2 mb-4">
                ...
                <div className="relative">
                    ...
                    <AnimatePresence>
                        {tooltip.visible && tooltip.type === "06" && (
                            ...
                            >
                                <div className="bg-card border border-warning-500/30 text-warning-200 text-[10px] font-bold px-4 py-2.5 rounded-xl shadow-2xl backdrop-blur-md flex items-center gap-2 whitespace-pre-line min-w-[220px] dark:bg-gray-900/95">
                                    <svg className="w-3.5 h-3.5 text-warning-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                    </svg>
                                    {tooltip.message}
                                    <div className="absolute top-full left-6 -translate-y-1/2 rotate-45 w-2 h-2 bg-card border-r border-b border-warning-500/30 dark:bg-gray-900"></div>
                                </div>
                            </motion.div>
                        )}
                    </AnimatePresence>
                    ...
                    <AnimatePresence>
                        {tooltip.visible && tooltip.type === "02" && (
                            <motion.div
                                ...
                                className="absolute bottom-full left-0 mb-3 z-50"
                            >
                                <div className="bg-card border border-secondary-blue-500/30 text-secondary-blue-200 text-[10px] font-bold px-4 py-2.5 rounded-xl shadow-2xl backdrop-blur-md flex items-center gap-2 whitespace-pre-line min-w-[220px] dark:bg-gray-900/95">
                                    <svg className="w-3.5 h-3.5 text-secondary-blue-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                    </svg>
                                    {tooltip.message}
                                    <div className="absolute top-full left-6 -translate-y-1/2 rotate-45 w-2 h-2 bg-card border-r border-b border-secondary-blue-500/30 dark:bg-gray-900"></div>
                                </div>
                            </motion.div>
                        )}
                    </AnimatePresence>
            ...
            >
                <div className={`relative flex items-center justify-center w-3 h-3`}>
                    <div className={`absolute inset-0 rounded-full border-2 transition-all duration-500 ${showOnlyAvailable ? "border-success-400" : "border-muted-foreground/40"}`}></div>

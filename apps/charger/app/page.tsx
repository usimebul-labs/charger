...
const StationCard = ({ station }: { station: Station }) => {
  ...
  return (
    <div className={`relative bg-slate-900/40 rounded-2xl p-6 sm:p-8 border backdrop-blur-3xl transition-all duration-500 overflow-hidden group flex flex-col items-center text-center
      ${isAvailable
    ...
    `}>
      ...
      <div className="flex flex-col items-center mb-6 relative z-10 w-full gap-1">
        <span className="text-[10px] sm:text-[11px] font-bold text-slate-500 tracking-widest uppercase opacity-70">
          ID {station.id}
        </span>
      </div>

      <div className="flex flex-col items-center gap-6 relative z-10 pb-2">
        <div className={`relative p-5 sm:p-6 rounded-2xl bg-slate-950/40 border-2 ${currentType.border} ${currentType.accent} shadow-2xl backdrop-blur-sm group-hover:scale-110 transition-all duration-500
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-[#020617] flex justify-center selection:bg-blue-500/30">
      {/* Mobile Frame Container */}
      <div className="w-full max-w-[480px] min-h-screen bg-slate-950 border-x border-slate-900 shadow-[0_0_100px_rgba(0,0,0,0.8)] relative flex flex-col pb-20">

        {/* Header */}
        <header className="bg-slate-950/80 border-b border-slate-800/50 sticky top-0 z-50 backdrop-blur-xl px-6 py-8 sm:px-10 overflow-hidden">
          ...
          <div className="relative flex flex-col md:flex-row md:items-center justify-between gap-6 px-6">
            <div>
              <h1 className="font-bold text-white tracking-tight flex flex-col leading-tight">
                <span className="text-[17px] sm:text-2xl text-[#0056FF] font-[family-name:var(--font-daki)]">DAOU DIGITAL SQUARE</span>
                <span className="text-[13px] sm:text-base text-slate-400 mt-1">전기차 충전 현황</span>
              </h1>
            ...
            <div className="flex items-center gap-4">
              ...
              >
                <div className={`relative flex items-center justify-center w-4 h-4`}>
                  <div className={`absolute inset-0 rounded-full border-2 transition-all duration-500 ${showOnlyAvailable ? "border-emerald-400" : "border-slate-700"}`}></div>
        ...
        <main className="flex-1 px-6 pt-8">
          <div className="space-y-16">
            {floors.map((floor) => {
              ...
              return (
                <section key={floor} className="space-y-8">
                  <div className="flex items-center gap-2">
                    <h2 className="text-[11px] font-black text-slate-400 tracking-wider">지하 {floor}층 14번 기둥</h2>
          ...
          {filteredStations.length === 0 && (
            <div className="flex flex-col items-center justify-center py-32 text-center">
              ...
              <button
                ...
                className="mt-8 px-6 py-2.5 rounded-full border border-slate-800 text-xs font-black tracking-widest uppercase hover:bg-slate-900 transition-colors"
              >
                전체 보기
              </button>
            </div>
